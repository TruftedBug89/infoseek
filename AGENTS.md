...
**Skill layout (skill-aware harnesses):** the repo root IS the skill dir —
opencode `~/.agents/skills/infoseek`, Claude Code `~/.claude/skills/infoseek`,
...
**Zero-decision entry point:** `await infoseek.run(q)` routes by query shape —
bare URL → page extraction (with archive fallback), `ask: ...` → context
...
- **Empty results are handled for you.** `run()`/`search` auto-retry once
  with the wide mix, then return an actionable hint instead of a bare `[]` —
  follow the hint (it names the exact next call to try) rather than giving up.
